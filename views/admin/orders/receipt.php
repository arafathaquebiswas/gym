...
<div class="invoice-wrapper">
  <?php if (!$isPdfMode): ?>
    ...
    <div class="no-print mb-3">
      <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
        <a href="<?= url('/admin/orders/' . $order['id']) ?>" class="btn btn-ps-outline btn-sm">
        ...
        <div class="d-flex gap-2">
          ...
          <a href="<?= url('/admin/orders/' . $order['id'] . '/pdf') ?>" class="btn btn-ps btn-sm">
  ...
  <!-- Invoice Header: Branding (Left) + Invoice Ref (Right) -->
  <table class="invoice-header-table">
    <tr>
      ...
      <td style="vertical-align: top; text-align: right;">
        ...
        <div style="font-size: 13px; font-weight: 700; color: #111827;">INV-<?= e($order['order_no']) ?></div>
        <div style="font-size: 11px; color: #4b5563; margin-top: 4px;">
          <div><strong>Order Ref:</strong> #<?= e($order['order_no']) ?></div>
          <div><strong>Date:</strong> <?= format_date($order['created_at'], 'd M Y, h:i A') ?></div>
        </div>
        <div style="margin-top: 6px;">
          <span class="invoice-badge <?= $isPaid ? 'invoice-badge-paid' : 'invoice-badge-pending' ?>">
            Payment: <?= e($paymentStatusLabel) ?>
          </span>
          <span class="invoice-badge invoice-badge-pending" style="background:#e0f2fe; color:#0369a1; border-color:#bae6fd;">
  ...
  <!-- Customer & Fulfillment Information (2-Column Box) -->
    ...
    <tr>
      <td class="info-box" style="width: 50%;">
        ...
        <div style="font-size: 12px; line-height: 1.5; color: #1f2937;">
          ...
          <?php if ($memberCode): ?>
            <div><span style="color:#6b7280;">Member ID:</span> <strong><?= e($memberCode) ?></strong></div>
          <?php endif; ?>
          <div><span style="color:#6b7280;">Phone:</span> <?= e($customerPhone) ?></div>
          <div><span style="color:#6b7280;">Email:</span> <?= e($customerEmail) ?></div>
          <?php if (!empty($order['delivery_address'])): ?>
            <div style="margin-top: 2px;"><span style="color:#6b7280;">Billing Address:</span> <?= e($order['delivery_address']) ?></div>
          <?php endif; ?>
        </div>
      </td>

      <td class="info-box" style="width: 50%;">
        <div class="info-box-title">FULFILLMENT & PAYMENT METHOD</div>
        <div style="font-size: 12px; line-height: 1.5; color: #1f2937;">
          <div><span style="color:#6b7280;">Fulfillment:</span> <strong><?= ucfirst((string) $order['fulfillment_method']) ?></strong></div>
          <?php if ($order['fulfillment_method'] === 'delivery'): ?>
            <div><span style="color:#6b7280;">Shipping Address:</span> <?= e(order_delivery_label($order)) ?></div>
            <?php if (!empty($order['time_slot_label'])): ?>
              <div><span style="color:#6b7280;">Time Slot:</span> <?= e($order['time_slot_label']) ?></div>
            <?php endif; ?>
          <?php else: ?>
            <div><span style="color:#6b7280;">Pickup Location:</span> Gym Main Front Desk</div>
            <?php if (!empty($order['pickup_pin'])): ?>
              <div><span style="color:#6b7280;">Pickup PIN:</span> <strong><?= e($order['pickup_pin']) ?></strong></div>
            <?php endif; ?>
          <?php endif; ?>
          <div style="margin-top: 6px; padding-top: 4px; border-top: 1px solid #e5e7eb;">
            <span style="color:#6b7280;">Payment Method:</span> <strong><?= e($paymentMethodLabel) ?></strong>
          </div>
        </div>
      </td>
    </tr>
  ...
  <table class="invoice-table">
    ...
    <tbody>
      <?php foreach ($items as $item): ?>
        ...
        ?>
        <tr>
          <td style="font-size: 11px; font-family: monospace; color: #4b5563;"><?= e($item['sku']) ?></td>
          ...
          <td>
            ...
            <?php if (!empty($item['discount_source'])): ?>
              <div style="font-size: 10px; color: #16a34a;"><?= e(ucfirst(str_replace('_', ' ', $item['discount_source']))) ?> Applied</div>
            <?php endif; ?>
          </td>
          <td style="text-align: center; font-weight: 600;"><?= $itemQty ?></td>
          <td style="text-align: right;"><?= money($itemUnitPrice) ?></td>
          <td style="text-align: right; color: #6b7280;"><?= $itemDiscount > 0 ? money($itemDiscount) : '—' ?></td>
      ...
      <td style="vertical-align: top; width: 55%; padding-right: 20px;">
        <div style="font-size: 11px; color: #4b5563; line-height: 1.4;">
          ...
          <div style="margin-top: 4px;">
            Thank you for choosing PowerSurge Gym! We appreciate your business. Please inspect your items upon delivery or pickup.
          </div>
          <div style="margin-top: 4px;">
      ...
      <td style="vertical-align: top; width: 45%;">
        <table class="totals-table">
          ...
          <?php if ($shipping > 0): ?>
            <tr>
              <td>Shipping Fee</td>
              <td class="amount"><?= money($shipping) ?></td>
            </tr>
          <?php else: ?>
            <tr>
              <td>Shipping Fee</td>
